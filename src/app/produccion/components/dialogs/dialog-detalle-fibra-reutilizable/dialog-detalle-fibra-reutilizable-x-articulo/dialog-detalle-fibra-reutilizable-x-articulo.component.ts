...
import { DetalleFibraReutilizableXArticulo } from '../../../../interfaces/balance-masas';

@Component({
  selector: 'app-dialog-detalle-fibra-reutilizable-x-articulo',
  templateUrl: './dialog-detalle-fibra-reutilizable-x-articulo.component.html',
  styleUrl: './dialog-detalle-fibra-reutilizable-x-articulo.component.css'
})
export class DialogDetalleFibraReutilizableXArticuloComponent implements OnInit{
  public articulo!: string;
  value = 'Clear me';
  dataSourceDetalleFibraXArticulo = new MatTableDataSource<DetalleFibraReutilizableXArticulo>();
  ...
  loadingDetalleFibraXArticulo: boolean = false;
  
  constructor(public dialog: MatDialog,@Inject(MAT_DIALOG_DATA) public data: any,
  private _consumoProvedorService: ConsumoProveedorServiceService,
  private _utilidadServicio: UtilidadService) {
   this.dataSourceDetalleFibraXArticulo = new MatTableDataSource(this.data.data);
   this.lote_madre = data.data_lote_madre;
   this.articulo = data.data_articulo;
   console.log('ut pata lote', this.lote_madre)
}
...
cargarDatosDetalleFibraReutilizable(){
  this.loadingDetalleFibraXArticulo = true;
  this._consumoProvedorService.getDetalleFibraReutilizable(this.lote_madre,this.data.data_articulo).subscribe(data =>{
    this.dataSourceDetalleFibraXArticulo.data = data
    this.loadingDetalleFibraXArticulo = false;
  }, (error: any) => {
    this.loadingDetalleFibraXArticulo = false;
...
getTotalKG(): number {
  return this.dataSourceDetalleFibraXArticulo.data
...
async exportTableToExcel() {
  if (this.dataSourceDetalleFibraXArticulo.data.length <= 0) {
  ...
  const data =  await this._utilidadServicio.formatoDetalleFibraReutilizablexArticulo(this.dataSourceDetalleFibraXArticulo);
